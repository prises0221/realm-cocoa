...
struct AsyncQueryCancelationToken {
    ...
    AsyncQueryCancelationToken(_impl::AsyncQuery* registration) : m_registration(registration) { }
...
private:
    _impl::AsyncQuery* m_registration = nullptr;
};
...
public:
    ...
    AsyncQueryCancelationToken asyncify(std::function<void (Results)> fn);
    AsyncQueryCancelationToken asyncify(Dispatcher dispatcher, std::function<void (Results)> fn);
